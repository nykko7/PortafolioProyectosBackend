...
var controller = {
    ...
    deleteProject: function(req, res) {
        ...
        Project.findByIdAndDelete(projectId, (err, projectRemoved) => {
            if (err) return res.status(500).send({ message: 'No se pudo remover el proyecto' });

            if (!projectRemoved) return res.status(404).send({ message: 'Nose puede eliminar ese proyecto.' });
        ...
        if (req.files) {
            ...
            if (fileExt == 'png' || fileExt == 'jpg' || fileExt == 'jpeg' || fileExt == 'gif') {
                Project.findByIdAndUpdate(projectId, { image: fileName }, { new: true }, (err, projectUpdated) => {
                    ...
                    if (!projectUpdated) return res.status(404).send({ message: 'El proyecto no existe y no se ha subido' });
